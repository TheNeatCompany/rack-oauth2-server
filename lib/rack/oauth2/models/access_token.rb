      ...
      class AccessToken
        class << self
          ...
          def from_token(token)
            Server.new_instance self, collection.find({ :_id=>token, :revoked=>nil }).first
          end

          # Find AccessToken from token. Returns revoked tokens.
          def from_all(token)
            Server.new_instance self, collection.find({ :_id=>token }).first
          end
          ...
          def for_client(client_id, offset = 0, limit = 100)
            ...
            collection.find(client_id: client_id).sort(created_at: 1).skip(offset).limit(limit).map{|token| Server.new_instance self, token }
          end
